...
Page::header("¿ELIMINAR ESTE PROYECTO?");
    /*Fecha*/
...
{
    header("location: galeria.php"); // ENCABEZADO DE LA PAGINA TIPOREDES
}
...
if(!empty($_POST))
{
	...
	try 
	{
		$sql = "DELETE FROM galerias WHERE Id_galeria = ?"; // CONSULTA A LA TABLA IMAGEN PRODUCTOS
	    $params = array($id);
...
if($valuando == 1){
      ...
      $bitacoriando = "CALL insertBitacora(5, 'Se elimino un proyecto de la galeria', ?, ?, ?)" ;
