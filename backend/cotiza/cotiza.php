...
if(!empty($_POST))
{
	...
	$sql = "SELECT * FROM cotizaciones WHERE cotizacion LIKE ? ORDER BY Id_cotizacion";
	$params = array("%$search%");
}
else
{
	$sql = "SELECT * FROM cotizaciones, clientes where clientes.Id_cliente=cotizaciones.Id_cliente AND  estadoCotizacion LIKE 0 ORDER BY Id_cotizacion";
	$params =  array();
}
	...
	<table class='col-md-12 col-lg-12 col-xs-12 col-lg-12 table-hover'>
					<thead>
			    		<tr>
				    		...
				    		<th><h5>FOTO DE COTIZACION</h5></th>
			    		</tr>
		    		</thead>
		    		<tbody>";
		foreach($data as $row)
		{
	        $tabla .=	"<tr class='active'>
	            			...
	            			<td class='tabla table-hover'>
	            				<a href='aceptar.php?id=".base64_encode($row['Id_cotizacion'])."' class='btn btn-info colordebotonmodificar'><i class='glyphicon glyphicon-pencil'></i> ACEPTAR</a>
								<a href='negar.php?id=".base64_encode($row['Id_cotizacion'])."' class='btn btn-danger colordebotoneliminar'><i class='glyphicon glyphicon-remove'></i> NEGAR</a>
	        			</tr>
	        			";
		}
		$tabla .= 	"</tbody>
