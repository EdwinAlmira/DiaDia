...
$hora_impresion = date("G:i:s");
	...
	class PDF extends FPDF 
	{
		function AcceptPageBreak()
		{
			...
			$this->SetFillColor(205,133,63);
			$this->SetFont('Arial','B',12);
			$this->SetX(10);
			$this->Cell(45,6,'Nombre',1,0,'C',1);
			$this->Cell(55,6,'Apellido',1,0,'C',1);
			$this->Cell(45,6,'Usuario',1,0,'C',1);
			$this->Cell(45,6,'correo',1,0,'C',1);
			$this->Ln();
		...
		function Header()
		{
			...
			$this->SetX(50);
			$this->SetFont('Arial','B',12);
			$this->Cell(0,6,'Reporte: Personal',0,1); 
			$this->SetX(50);
			$this->SetFont('Arial','',12);
	...
	foreach($resultado as $row)
	{
		...
		$pdf->SetFont('Arial','',12);
		$pdf->SetX(8);
		$pdf->Cell(45,6, utf8_decode($row['nombrePersonal']),1,0,'C');;
		$pdf->Cell(55,6, utf8_decode($row['apellidoPersonal']),1,0,'C');
		$pdf->Cell(45,6, utf8_decode($row['usuario']),1,0,'C');
		$pdf->Cell(50,6, utf8_decode($row['correo_personal']),1,1,'C');
	}
	$pdf->Output();
