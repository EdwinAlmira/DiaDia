...
if(!empty($_POST))
{
                ...
                else if(trim($_POST['correo'])!= "")
                {
                    ...
                    else if(trim($_POST['usuario'])!= "")
                    {
                        ...
                        {   
                            ...
                            $sql = "INSERT INTO personal(nombrePersonal, apellidoPersonal,correo_personal,usuario,Id_cargo,fechaIngreso, horaIngreso,clave_personal) VALUES(?,?,?,?,?,?,?, ?)";
                        ...
                        {
                             $sql = "UPDATE personal SET nombrePersonal = ?, apellidoPersonal = ?, correo_personal = ?, usuario = ?, Id_cargo = ?, fechaIngreso = ?, horaIngreso = ? WHERE Id_personal = ?";
    ...
    {
        print("<div class='alert alert-danger col-xs-12 col-md-8 col-lg-5' role='alert'><b> AVISO:</b></i> <b>EL USUARIO DIGITADO YA EXISTE COMO REGISTRADO.</b></div>");
    }
}

if($valuando == 1){
      $usuariando = $_SESSION['Id_personal'];
      $bitacoriando = "CALL insertBitacora(3, 'Se agrego un nuevo integrante del personal', ?, ?, ?)" ;
    ...
    if($valuando == 2){
        $usuariando = $_SESSION['Id_personal'];
        $bitacoriando = "CALL insertBitacora(4, 'Se modifico un integrante del personal', ?, ?, ?)" ;
        $parametrando = array($usuariando, $fechaIngreso, $horaIngreso);
        Database::executeRow($bitacoriando, $parametrando);
    }
}
