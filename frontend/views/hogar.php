	...
	<br>
	 ...
	 <h1 class="text-center fuente fuentecolor"> <i>HOGAR </i></h1><br> 
	 <hr>
	 </div>
		...
		<div class="container-fluid" id="principalP">
			...
			<div class="col-sm-2 col-md-2 postz wow slideInLeft" data-wow-offset="300"  data-wow-iteration="1" id="" >
				...
				<?php
						...
						$sql = "SELECT subcategoria, id_subcategoria from subcategorias where Id_categoria=1";
				...
				<form action='' method='post' class="navbar-form navbar-right" role="search">
	                <input id='buscar' type='text'  autocomplete="off" placeholder = 'Buscar categorías' onkeypress="return soloLetras(event)" class='validate col-md-12'/><br><br>
	                <input type='submit' name='buscar' value='Buscar' class='btn btn-hola  col-md-offset-4 col-xs-offset-2 colordebotonaceptar enblanco' />
				</form>
	        ...
	        <?php
	        	if(isset($_SESSION['Id_cliente'])){
	                ...
	                echo "<a class='fuente' href='views/login.php'><i '></i><span class=' '></span> </a><br>";
	            } 
					...
					if(!empty($_POST))
							{
								if(isset($_POST['filtrar']) and $_POST['filtrar'] == 'si'){
									...
									$sql = "SELECT productos.Id_producto, nombreProdu, miniDescrip, precio,imagen, productos.id_subcategoria, subcategorias.Id_categoria FROM productos, categorias, subcategorias WHERE  productos.id_subcategoria = subcategorias.id_subcategoria and subcategorias.Id_categoria=categorias.Id_categoria AND subcategorias.Id_categoria=1 AND estadoProducto = 1 and subcategorias.id_subcategoria = ? ORDER BY nombreProdu";
									$params = array($search);
				  	            }
					            else{
								$searchi = trim($_POST['buscar']);
									if($searchi != ""){

										$sql = "SELECT productos.Id_producto, nombreProdu, miniDescrip, precio,imagen, productos.id_subcategoria, subcategorias.Id_categoria FROM productos, categorias, subcategorias WHERE productos.id_subcategoria = subcategorias.id_subcategoria and subcategorias.Id_categoria=categorias.Id_categoria AND subcategorias.Id_categoria=1 AND estadoProducto = 1 and nombreProdu LIKE ? ORDER BY nombreProdu";
										$params = array("%$searchi%");
									}
									else{
										$sql = "SELECT productos.Id_producto, nombreProdu, miniDescrip, precio,imagen, productos.id_subcategoria,subcategorias.Id_categoria FROM productos, categorias, subcategorias WHERE  productos.id_subcategoria = subcategorias.id_subcategoria and subcategorias.Id_categoria=categorias.Id_categoria AND subcategorias.Id_categoria=1 AND estadoProducto = 1 ORDER BY nombreProdu";
										$params = null;
									}
								}
							}
							  	...
							  	<div class='thumbnail'>
							    			...
							    			<div class='caption'>
							      				...
							      				<form action='' method='post'>
				                                    ...
				                                    <input type='hidden' name='idp'  value='$idproducto' />
				                                    <input type='hidden' name='buscar'  value='' /> <!--  id producto -->";
				                                     if(isset($_SESSION['usuario'])){
