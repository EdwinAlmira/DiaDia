...
<br>
 ...
 <h1 class="text-center fuente fuentecolor"> <i>DECORACION</i></h1><br> 
 <hr>
 </div>
	...
	<div class="container-fluid" id="principalP">
		...
		<div class="col-sm-12 col-md-offset-10 col-md-2 postz" id="barranueva">
			...
			<?php
					...
					$sql = "SELECT subcategoria, id_subcategoria from subcategorias where Id_categoria=4";
            ...
            function alcarro($idprodu, $cliente) {
				...
				$mlg = Database::getRows($sqlito, $parameeshta);
		            ...
		            Database::executeRow($sql, $params);
							...
							//Aqi un sweet alert que ya se habia agregado al HIJUEPUTA carro
				}		
            }
					...
					<?php
						...
						if(!empty($_POST))
						{
							if(isset($_POST['filtrar']) and $_POST['filtrar'] == 'si'){
								...
								$sql = "SELECT productos.Id_producto, nombreProdu, miniDescrip, precio,imagen, productos.id_subcategoria, subcategorias.Id_categoria FROM productos, categorias, subcategorias WHERE  productos.id_subcategoria = subcategorias.id_subcategoria and subcategorias.Id_categoria=categorias.Id_categoria AND subcategorias.Id_categoria=4 AND estadoProducto = 1 and subcategorias.id_subcategoria = ? ORDER BY nombreProdu";
								$params = array($search);
				            }
				            else{
							$search = trim($_POST['buscar']);
							$sql = "SELECT productos.Id_producto, nombreProdu, miniDescrip, precio,imagen, productos.id_subcategoria, subcategorias.Id_categoria FROM productos, categorias, subcategorias WHERE productos.id_subcategoria = subcategorias.id_subcategoria and subcategorias.Id_categoria=categorias.Id_categoria AND subcategorias.Id_categoria=4 AND estadoProducto = 1 and nombreProdu LIKE ? ORDER BY nombreProdu";
							$params = array("%$search%");
							}
						}
						else
						{
						$sql = "SELECT productos.Id_producto, nombreProdu, miniDescrip, precio,imagen , productos.id_subcategoria,subcategorias.Id_categoria FROM productos, categorias, subcategorias WHERE productos.id_subcategoria = subcategorias.id_subcategoria and subcategorias.Id_categoria=categorias.Id_categoria AND subcategorias.Id_categoria=4 AND estadoProducto = 1 ORDER BY nombreProdu";
						$params = null;
						}
